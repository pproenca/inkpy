...
from inkpy.backend.tui_backend import HostContext, TUIBackend
...
def test_commit_update_diffs_attributes():
    ...
    node = backend.create_instance(
        "ink-box", {"id": "test", "data": "old"}, None, HostContext(is_inside_text=False)
    )

    # Update with changed props
    backend.commit_update(
        node, "ink-box", {"id": "test", "data": "old"}, {"id": "test", "data": "new"}
    )
...
def test_commit_update_diffs_styles():
    ...
    node = backend.create_instance(
        "ink-box", {"style": {"padding": 1}}, None, HostContext(is_inside_text=False)
    )
...
def test_static_element_detection():
    ...
    static_node = backend.create_instance(
        "ink-box", {"internal_static": True}, root_node, HostContext(is_inside_text=False)
    )
...
def test_yoga_node_cleanup_on_remove():
    ...
    original_free = (
        child.yoga_node.free_recursive
