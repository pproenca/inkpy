...
class TestInputLoopInitialization:
    ...
    def test_input_loop_not_started_in_state_setter(self):
        ...
        assert (
            "input_loop_started_container[0] = True" in source or "_start_input_loop()" in source
    ...
    def test_handle_set_raw_mode_graceful_fallback(self):
        ...
        assert (
            "Warning: Raw mode not supported" in source or "return" in source
...
class TestEffectCleanupTiming:
    ...
    async def test_cleanup_runs_on_unmount_not_during_lifecycle(self):
        ...
        async with layout:
            ...
            assert (
                len(cleanups_during_lifecycle) == 0
...
class TestWaitUntilExitLifecycle:
    ...
    async def test_ink_run_interactive_lifecycle_exists(self):
        ...
        assert hasattr(
            Ink, "_run_interactive_lifecycle"
    ...
    async def test_wait_until_exit_calls_interactive_lifecycle(self):
        ...
        assert (
            "_run_interactive_lifecycle" in source
...
class TestEventEmitterConnection:
    ...
    async def test_input_handler_connected_to_emitter(self):
        ...
        stdin_ctx = {"internal_eventEmitter": emitter, "internal_exitOnCtrlC": True}
...
class TestRawModeStartsInputLoop:
    ...
    def test_app_has_input_loop_tracking(self):
        ...
        assert (
            "input_loop_started" in source or "loop_control" in source
    ...
    def test_input_loop_restores_terminal_on_exit(self):
        ...
        assert "termios.tcsetattr" in source, "Input loop should restore terminal settings on exit"
...
class TestNoDuplicateRendering:
    ...
    def test_separate_contexts_for_sync_and_interactive(self):
        ...
        assert (
            "_do_sync_render" in render_source
        ...
        assert (
            "Layout(self._app_component)" in lifecycle_source
...
class TestOutputProcessingPreserved:
    ...
    def test_cbreak_mode_preserves_output_processing(self):
        ...
        assert "termios.tcgetattr" in source, "Should use termios.tcgetattr to get current settings"
        assert "termios.tcsetattr" in source, "Should use termios.tcsetattr to set new settings"

        # Must disable ICANON (canonical mode) for character-by-character input
        assert "ICANON" in source, "Should disable ICANON flag for non-buffered input"

        # Must disable ECHO to prevent input echoing
        assert "ECHO" in source, "Should disable ECHO flag to prevent keystroke echoing"
    ...
    def test_output_newlines_work_correctly(self):
        ...
        assert "new_settings[3]" in source, "Should modify lflag (index 3) for input settings"
        ...
        for line in lines:
            if "new_settings[1]" in line and "=" in line:
                ...
                assert False, f"Should NOT modify output flags (new_settings[1]): {line}"
