...
def _get_code_excerpt(
    file_path: str, line_num: int, context_lines: int = 3
...
def ErrorOverview(error: Exception):
    ...
    error_header = Box(
        children=[Text(" ERROR ", backgroundColor="red", color="white"), Text(f" {error!s}")]
    )
    ...
    if origin and file_path:
        ...
        children.append(Box(style={"marginTop": 1}, children=[Text(location_text, dimColor=True)]))
    ...
    if origin and excerpt:
        ...
        for line_num, line_content in excerpt:
            ...
            excerpt_children.append(Box(children=[line_num_component, line_content_component]))

        children.append(
            Box(style={"marginTop": 1, "flexDirection": "column"}, children=excerpt_children)
        )
    ...
    if stack_lines:
                ...
                stack_children.append(
                    Box(
                        children=[
                            Text("- ", dimColor=True),
                            Text(
                                parsed_line.get("function", "<unknown>"), dimColor=True, bold=True
                            ),
                            Text(
                                f" ({file_location})",
                                dimColor=True,
        ...
        if stack_children:
            children.append(
                Box(style={"marginTop": 1, "flexDirection": "column"}, children=stack_children)
            )

    return Box(style={"flexDirection": "column", "padding": 1}, children=children)
