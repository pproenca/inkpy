...
def App(
    ...
    def handle_set_raw_mode(is_enabled: bool):
        ...
        if not is_raw_mode_supported():
            ...
            print(
                f"Warning: Raw mode not supported on stdin (isatty={stdin.isatty() if hasattr(stdin, 'isatty') else 'N/A'})",
                file=_sys.stderr,
            )
            return
                ...
                while not loop_control["event"].is_set():
                    ...
                    if hasattr(stdin, "fileno"):
                        ...
                        if hasattr(stdin, "read"):
                            try:
                                chunk = (
                                    stdin.read(1) if select.select([stdin], [], [], 0)[0] else None
                                )
                                if chunk:
                                    handle_input(chunk)
                                    event_emitter.emit("input", chunk)
    ...
    def focus_next():
        if not focusables:
            return
        current_index = next(
            (i for i, f in enumerate(focusables) if f["id"] == active_focus_id), -1
        )
    ...
    # Context values
    app_context_value = {"exit": lambda error=None: on_exit(error) if on_exit else None}
    ...
    stdout_context_value = {"stdout": stdout, "write": write_to_stdout}

    stderr_context_value = {"stderr": stderr, "write": write_to_stderr}
    ...
    return html.div(
        AppContext(
            StdinContext(
                StdoutContext(
                    StderrContext(
                        FocusContext(inner_content, value=focus_context_value),
