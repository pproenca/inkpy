...
class NodeView(poga.PogaView):
    ...
    def size_that_fits(self, width: float, height: float) -> tuple[float, float]:
        ...
        w = (
            self._last_valid_width
            if self._last_valid_width is not None
            else (0.0 if math.isnan(width) else width)
        )
        h = (
            self._last_valid_height
            if self._last_valid_height is not None
            else (0.0 if math.isnan(height) else height)
        )
        return (w, h)
