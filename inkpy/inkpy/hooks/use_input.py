...
from ..input.keypress import NON_ALPHANUMERIC_KEYS, Key, parse_keypress
from .use_stdin import use_stdin


def use_input(input_handler: Callable[[str, Key], None], is_active: bool = True) -> None:
    """
    ...
    def setup_raw_mode():
        ...
        set_raw_mode = (
            stdin_ctx.get("set_raw_mode")
            if isinstance(stdin_ctx, dict)
            else getattr(stdin_ctx, "set_raw_mode", None)
        )
        if set_raw_mode:
            set_raw_mode(True)
...
def _setup_input_listener(stdin_ctx, input_handler: Callable[[str, Key], None]):
    ...
    if event_emitter:
        if hasattr(event_emitter, "on"):
            ...
            return (
                lambda: event_emitter.removeListener("input", handle_data)
                if hasattr(event_emitter, "removeListener")
                else None
            )
        elif hasattr(event_emitter, "add_listener"):
            event_emitter.add_listener("input", handle_data)
            return (
                lambda: event_emitter.remove_listener("input", handle_data)
                if hasattr(event_emitter, "remove_listener")
                else None
            )
