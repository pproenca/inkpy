...
from .renderer.ansi_tokenize import slice_ansi, string_width, tokenize_ansi
...
def _truncate_text(text: str, max_width: int, truncate_type: str) -> str:
    ...
    if "\n" in text:
        lines = text.split("\n")
        truncated_lines = [_truncate_text(line, max_width, truncate_type) for line in lines]
    ...
    if truncated_tokens:
        ...
        if last_token.get("type") == "ansi" and last_token.get("value") != "\x1b[0m":
            ...
            has_ansi_in_truncated = any(
                t.get("type") == "ansi" and t.get("value") != "\x1b[0m" for t in truncated_tokens
            )
