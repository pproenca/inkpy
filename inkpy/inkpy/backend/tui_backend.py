...
class TUIBackend:
    ...
    def create_instance(
        ...
        node_type = (
            "ink-virtual-text"
            if (original_type == "ink-text" and host_context.is_inside_text)
            else original_type
        )
    ...
    def create_text_instance(
        self, text: str, root_node: Optional[DOMElement], host_context: HostContext
    ) -> TextNode:
    ...
    def commit_update(
        self, node: DOMElement, node_type: str, old_props: dict[str, Any], new_props: dict[str, Any]
    ):
        ...
        style_diff = _diff(old_props.get("style", {}), new_props.get("style", {}))
