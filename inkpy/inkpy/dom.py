...
def insert_before_node(
    parent: DOMElement,
    new_child: Union[DOMElement, TextNode],
    before_child: Union[DOMElement, TextNode],
):
...
# Helper functions
def measure_text_node(
    node: Union[DOMElement, TextNode], width: float, height: float
) -> dict[str, float]:
    ...
    return {
        "width": float(wrapped_dimensions["width"]),
        "height": float(wrapped_dimensions["height"]),
    }
