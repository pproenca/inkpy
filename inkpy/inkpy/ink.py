...
from inkpy.dom import DOMElement, create_node
from inkpy.is_in_ci import is_in_ci
from inkpy.log_update import LogUpdate, create_log_update
...
class Ink:
    ...
    def __init__(
        ...
        self._render_throttle_ms = max(1, int(1000 / max_fps)) if max_fps > 0 else 0
        ...
        self.log: LogUpdate = create_log_update(
            stdout, show_cursor=False, incremental=incremental_rendering
        )
        ...
        if patch_console and not debug:
            ...
            self.restore_console = patch_console(stdout, stderr, console_callback)
    ...
    def on_render(self):
        ...
        if self.is_screen_reader_enabled:
            if has_static_output:
                ...
                erase_code = (
                    erase_lines(self.last_output_height) if self.last_output_height > 0 else ""
                )
